...
- The `disableGlobal` option of the [class name generator](/css-in-js/api/#creategenerateclassname-options-class-name-generator) is `false` (default).

These conditions are met with the most common use cases of `@material-ui/core`.
We are providing default values for the `classes` API. For instance:
...
This style sheet generates the following class names you can override:
